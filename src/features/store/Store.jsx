...
const filteredProducts = useStore(state =>state.products).filter(product => product.category === pageProduct)

  return (
    <Box mt="120px">
    
{  pageProduct==="Weekend Boot"  &&  <Box as="main" position="relative">
        <Img
...
src={require(`../../assets/images/weekendBootsHero,w_941.webp`)}
        ...
        >
          <Typography as="h2"  sx={{fontSize:{xs:"1.6rem",sm:"calc(2rem + 4vw)"}, fontWeight:"700"}}>{t("store.storeBootsHero")}</Typography>
      ...
      {  pageProduct==="Terrus Clog"  &&  <Box as="main" position="relative">
        <Img
          sizes="(max-width: 1280px) 50vw, 1280px"
...
src={require(`../../assets/images/terrusClogsHero,w_941.webp`)}
        ...
        <Stack
          ...
          sx={{  top:{xs:"2vw", sm:"-12vw"}}}
        >
          <Typography as="h2" sx={{fontSize:{xs:"1.6rem",sm:"calc(2rem + 4vw)"},  fontWeight:"700"}}>{t("store.storeClogsHero")}</Typography>
          <Typography  as="p" sx={{fontSize:{xs:"0.8rem",sm:"calc(0.2rem + 1vw)"}}}>
      ...
      <Stack mt={2} mb={7} spacing={1} alignItems="center">
         <Typography variant="h2" width="90%" textAlign="center">{t(`store.${pageTitle}`)}</Typography>
        {pageParagraph && <Typography variant="body2" as="p" width="70%" textAlign="center">{t(`store.${pageParagraph}`)}</Typography>}
      </Stack>
